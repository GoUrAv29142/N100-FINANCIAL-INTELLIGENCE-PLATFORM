...
- Capital Allocation Label: this script computes it directly from the
  ...
  computation is the fallback. This keeps Day 31 self-contained ahead of
  the Day 32 reconciliation task.
...
def load_existing_capital_allocation():
    """Reuse Sprint 2's capital_allocation.csv if present, for consistency
    ahead of Day 32's reconciliation. Returns {company_id: latest_label} or {}."""
...
def evaluate_company(company_id, sector, cf_hist, bs_hist, ratio_hist, existing_labels):
    ...
    # ---------- CFO Quality Score (5yr avg of cfo_pat_ratio) ----------
    if not ratio_hist.empty:
        ...
        # ---------- FCF Conversion (FCF / operating_profit x100) ----------
    ...
    # ---------- Distress Signal + Capital Allocation (latest raw cashflow year) ----------
